__author__ = "monkeyman192" # modified to include directories
__version__ = "1.0.1"
...
from Resources.OodleCompressor import OodleCompressor, OodleDecompressionError
from Resources.utils import OSCONST
...
class HGPakFile():
    ...
    def unpack(
        ...
        directories: Optional[list[str]] = None,  # NEW: accept directories
    ) -> int:
        i = 0
        ...
        if directories is not None:
            ...
            for dir_ in directories:
                dir_ = dir_.replace("\\", "/")  # normalize path
                dir_ = dir_.lower()
                for f in self.files:
                    if dir_ in f.lower():  # check if file is inside directory
                        files.add(f)
